...
export default class RideRepositoryDatabase implements RideRepository {
  ...
  async save(ride: Ride): Promise<void> {
    await this.connection.query(
      `insert into public.ride (ride_id, passenger_id, from_lat, from_long, to_lat, to_long, status, request_date)  values($1, $2, $3, $3, $5, $6, $7, $8)`,
  ...
  async get(rideId: string): Promise<Ride> {
    const [rideData] = await this.connection.query("select * from public.ride where ride_id = $1", [rideId]);
  ...
  async update(ride: Ride): Promise<void> {
    await this.connection.query(
      `update public.ride set driver_id = $1, status = $2, accept_date = $3, start_date = $4, end_date = $5 where ride_id = $6`,
