...
test("Não Deve cadastrar o passageiro com email invalido", async function () {
  const input = {
      name: "John Doe",
...
test("Deve obter o passageiro", async function () {
  ...
  const usecase1 = new CreatePassenger(new PassengerRepositoryDatabase(connection), new UserRepositoryDatabase(connection))
  const output1 = await usecase1.execute(input)
  const usecase2 = new GetPassenger(new PassengerRepositoryDatabase(connection), new UserRepositoryDatabase(connection))
